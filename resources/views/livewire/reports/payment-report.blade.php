                    ...
                    <div class="row">
                        <div class="col-sm-12 col-md-5 my-2">
                            <div class="input-group input-group-sm">
                                ...
                                <input type="date" class="form-control" aria-label="Search" aria-describedby="search" wire:model.lazy="start_date">
                                <span class="input-group-text" id="search">TO</span>
                                <input type="date" class="form-control" aria-label="Search" aria-describedby="search" wire:model.lazy="end_date">
                            </div>
                        </div>
                        <div class="col-sm-12 col-md-7 my-2">
                            <div class="input-group input-group-sm">
                                <span class="input-group-text" id="search"><i class="fa-solid fa-magnifying-glass"></i></span>
                                <input type="text" class="form-control" placeholder="Search" aria-label="Search" aria-describedby="search" wire:model.lazy="search">
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="table table-sm table-hover table-striped table-bordered">
                            ...
                            <tbody>
                                @forelse ($reports as $row)
                                    <tr style="cursor: pointer" onclick="update_payment('{{ $row->id }}', '{{ $row->status }}')">
                                        <td>
                                        ...
                                        <td>
                                            <span
                                                class="badge text-small @if($row->status == 'Pending') bg-secondary @elseif($row->status == 'Success') bg-primary @elseif($row->status == 'Commissioned') bg-success @elseif($row->status == 'Cancelled') bg-danger @endif">{{ $row->status }}</span>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
